...
/**
 * A file system manager, that recursively monitors a given share directory, and
 * emits events when files and directories change. The order that events are
 * emitted in is natural in the sense that may be reproduced in the order
 * supplied on a remote system without reordering required. E.g. if a nested
 * directory structure is deleted, then the deepest deletions are always given
 * first, up to the shallowest deletions. Similarly when creating nested
 * directories, and for files that appear in directories. <br/>
 * The file system manager also provides an API for safely making modifications
 * to the files and directories in the share directory:
...
public class FileSystemManager extends Thread {
	...
	/**
	 * The special suffix on file names for loading files. Any files in the share
	 * directory with this suffix will never generate file system events, as they
	 * are ignored by the file system monitor.
	 */
	...
	/**
	 * Describes the case when a file is created/deleted/modified or when a
	 * directory is created/deleted.
	 * <li>{@link #name}</li>
	 * <li>{@link #path}</li>
	 * <li>{@link #pathName}</li>
	 * <li>{@link #event}</li>
	...
	public class FileSystemEvent {
		/**
		 * The path (not including the name) of the file/directory, including the share
		 * directory.
		 */
		public String path;
		/**
		...
		public String name;
		/**
		 * The pathname of the file/directory, <i>relative</i> to the share directory.
		 */
	...
	/**
	 * Construct a new file system manager. If the supplied share directory is not a
	 * directory that exists then the constructor will return without starting the
	 * monitor thread.
	 * 
	 * @param root               The pathname to the root directory to share, called
	 *                           the share directory.
	 * @param fileSystemObserver The observer of the file system events, which must
	 *                           implement {@link FileSystemObserver}.
	 * @throws IOException              Thrown if an initial scan of the share
	 *                                  directory fails.
	 * @throws NoSuchAlgorithmException Thrown if the MD5 hash algorithm is not
	 *                                  available.
	 */
	...
	/**
	 * Returns true if the path name is "safe" to be used. Unsafe names should not
	 * be used as they may access data outside of the share directory.
	 * 
	 * @param pathName The path name to test for safeness, relative to the share
	 *                 directory.
	 * @return boolean True if the path name is safe to use, false otherwise
	 *         including if there was an IO error accessing the file system.
	 */
	 ...
	 * 
	 * @param pathName The name of the directory to test for, relative to the share
	 *                 directory.
	...
	/**
	 * Attempts to make a directory, the parent of the directory must already exist.
	 * 
	 * @param pathName The name of the directory to make, relative to the share
	 *                 directory.
	...
	public boolean makeDirectory(String pathName) {
		...
		synchronized (this) {
			File file = new File(root + FileSystems.getDefault().getSeparator() + pathName);
			addDir(root + FileSystems.getDefault().getSeparator() + pathName);
			return file.mkdir();
		}
	}

	/**
	 * Attempts to delete a directory, the directory must be empty.
	 * 
	 * @param pathName The name of the directory to delete, relative to the share
	 *                 directory.
	...
	public boolean deleteDirectory(String pathName) {
		...
		File file = new File(root + FileSystems.getDefault().getSeparator() + pathName);
		if (file.isDirectory()) {
			dropDir(root + FileSystems.getDefault().getSeparator() + pathName);
			return file.delete();
		} else
			return false;
	}
	 ...
	 * 
	 * @param pathName The name of the file to test for, relative to the share
	 *                 directory.
	 * @return boolean True if the file exists. In the case of a file that is being
	 *         created and currently loading, returns false. In the case of a file
	 *         that is being modified and currently loading, returns true.
	 */
	 ...
	 * @return True if the file exists. In the case of a file that is being created
	 *         and currently loading, returns false. In the case of a file that is
	 *         being modified and currently loading, returns true against the
	 *         existing file.
	 */
	public boolean fileNameExists(String pathName, String md5) {
		...
		synchronized (this) {
			...
			return watchedFiles.containsKey(fullPathName) && watchedFiles.get(fullPathName).md5.equals(md5);
		}
	}

	/**
	 * Attempt to delete a file. The file must exist and it must have a last
	 * modified timestamp less than or equal to that supplied.
	 * 
	 * @param pathName     The name of the file to delete, relative to the share
	 *                     directory.
	...
	/**
	 * Create a file loader for given file name. The file name must not already
	 * exist, otherwise use {@link #modifyFileLoader(String, String, long)}. The
	 * file loader maintains a place holder file with prefix {@link #loadingSuffix}
	 * on its filename, called a <i>loader file</i>. Such files never generate file
	 * system events. The file loader can be subsequently accessed via the given
	 * name using {@link #writeFile(String, ByteBuffer, long)},
	 * {@link #checkWriteComplete(String)} and {@link #checkShortcut(String)}.
	 * 
	 * @param pathName     The name of the file to create, when loading is complete,
	 *                     relative to the share directory.
	 * @param md5          The MD5 hash of the content that the file contents
	 *                     <i>must</i> match for file loading to be considered
	 *                     complete.
	 ...
	 * @throws IOException              if any exceptions arose as the result of
	...
	/**
	 * Requests the file loader for the associated file name to write the supplied
	 * byte buffer at the supplied position in the loader file.
	 * 
	 * @param pathName The name of the file to which the file loader is associated
	 *                 (no special prefix).
	 * @param src      The bytes to be written.
	 * @param position The position to write the bytes.
	 * @return True if successfully written, false if there was no associated file
	 *         loader for the given name.
	 ...
	 * @return A {@link java.nio.ByteBuffer} if the bytes are successfully read,
	 *         otherwise null if there was no such file with that content.
	 * @throws IOException              If there were any problems accessing the
	 *                                  file system.
				...
				for (String attempt : hashMap.get(md5)) {
					try {
						...
						if (currentMd5.equals(md5)) {
							...
							raf.close();
							if (read < length)
								throw new IOException("did not read everything expected");
							return bb;
						}
						lock.release();
						channel.close();
	...
	/**
	 * Requests the file loader for the associated file name to check if all of the
	 * content for the file has been written. It does this by checking the MD5 hash
	 * of the written bytes to see if they match the MD5 hash of the intended file.
	 * <b>NOTE:</b> Hashing the file contents is time consuming and so this check
	 * should not be done often. If the check succeeds then the loader file is
	 * renamed to the name the file it should be (i.e. without the prefix), and the
	 * loader is no longer accessible. If a file exists in its intended place at
	 * this point in time then the file is deleted.
	 * 
	 * @param pathName The name of the file to check if loading has completed.
	 * @return True if the file was completed, false if not and the loader is still
	 *         waiting for more data.
	 * @throws NoSuchAlgorithmException If the MD5 hash algorithm is not available,
	 *                                  the loader is no longer available in this
	 *                                  case.
	 * @throws IOException              If there was a problem accessing the file
	 *                                  system, the loader is no longer available in
	 *                                  this case.
	 */
	...
	/**
	 * Should be called directly after creating a file loader, but can be called at
	 * any time. Requests the file loader to check if another file already exists
	 * with the same content, and if so, uses that file's content (i.e. does a copy)
	 * to create the intended file. The file loader is then complete and is no
	 * longer accessible. This is much faster than transferring the bytes of the
	 ...
	 *                                  the loader is no longer available in this
	 *                                  case.
	 * @throws IOException              If there were any errors accessing the file
	 *                                  system, the loader is no longer available in
	 *                                  this case.
	 */
	...
	/**
	 * Called to create a file loader in the case when a file name already exists.
	 * The existing file must have a last modified timestamp that is less than or
	 * equal to the supplied one. See {@link #createFileLoader(String, String)} for
	 * more details about the file loader.
	 * 
	 * @param pathName     The name of the file to modify.
	 * @param md5          The MD5 hash of the content that the loaded file
	 *                     <i>must</i> have in order for the loading to complete.
	 * @param lastModified The existing file's timestamp must be less than this time
	 *                     stamp for the loader to be successfully created.
	...
	public boolean modifyFileLoader(String pathName, String md5, long lastModified) throws IOException {
		...
		synchronized (this) {
			...
			if (loadingFiles.containsKey(fullPathName))
				return false;
			if (watchedFiles.containsKey(fullPathName) && watchedFiles.get(fullPathName).lastModified <= lastModified) {
				loadingFiles.put(fullPathName,
						new FileLoader(fullPathName, md5, watchedFiles.get(fullPathName).fileSize, lastModified));
			} else
				return false;
		}
		return true;
	...
	/**
	 * Cancel a file loader. Removes the file loader if present, including the
	 * loader file. No other actions are taken.
	 * 
	 * @param pathName The name of the file loader, i.e. the associated file it was
	 *                 trying to load.
	 * @return True if the file loader existed and was cancelled without problem,
	 *         false otherwise. The loader is no longer available in any case.
	 * @throws IOException if there was a problem accessing the file system, the
	 *                     loader is no longer available in this case.
	 */
	...
	public ArrayList<FileSystemEvent> generateSyncEvents() {
		synchronized (this) {
			...
			for (String pathname : keys) {
				File file = new File(pathname);
				pathevents.add(new FileSystemEvent(file.getParent(), file.getName(), EVENT.FILE_CREATE,
						watchedFiles.get(pathname)));
			}
			return pathevents;
		}
	}
	...
	private class FileLoader {
		...
		public void writeFile(ByteBuffer src, long position) throws IOException {
			if (position > length)
				throw new IOException("trying to write bytes beyond what is expected");
		...
		public boolean checkWriteComplete() throws NoSuchAlgorithmException, IOException {
			...
			if (currentMd5.equals(md5)) {
				...
				addFile(pathName, new FileDescriptor(lastModified, md5, dest.length()));
				return true;
			}
			return false;
		}
	...
	private String hashFile(File file, String name, long lastModified, RandomAccessFile raf)
			throws NoSuchAlgorithmException, IOException {
		log.info("hashing file " + name);
		if (lastModified != 0 && lastModified == file.lastModified()) {
			return watchedFiles.get(name).md5;
		...
		if (file.isFile()) {
			...
			if (watchedFiles.containsKey(name)) {
				if (lastModified != watchedFiles.get(name).lastModified) {
					...
					FileSystemEvent pe = new FileSystemEvent(file.getParent(), file.getName(), EVENT.FILE_MODIFY,
							watchedFiles.get(name));
					pathEvents.add(pe);
				} else {
					// do nothing
				}
				...
				FileSystemEvent pe = new FileSystemEvent(file.getParent(), file.getName(), EVENT.FILE_CREATE,
						watchedFiles.get(name));
				pathEvents.add(pe);
			}
	...
	private void removeHash(String name) {
		...
		hs.remove(name);
		if (hs.size() == 0)
			hs.remove(watchedFiles.get(name).md5);
	}
